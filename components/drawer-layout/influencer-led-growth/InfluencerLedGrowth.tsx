...
const InfluencerLedGrowth: React.FC<{ variant: Variant }> = ({ variant }) => {
    ...
    const { subDrawerOpen, openSubDrawer, toggleSubDrawer } = useSidebarCollapsed();
    ...
    if (variant === "rail") {
        return (
            <Pressable
                onPress={toggleSubDrawer}
                onHoverIn={() => setHovered(true)}
                onHoverOut={() => setHovered(false)}
                style={styles.railTriggerWrapper}
                accessibilityRole="button"
                accessibilityState={{ selected: subDrawerOpen }}
                accessibilityLabel="Influencer Led Growth"
            >
                <RNView
                    style={[
                        styles.railTriggerInner,
                        (subDrawerOpen || hovered) && styles.railTriggerActive,
                    ]}
                >
                    <FontAwesomeIcon
                        icon={faUsers}
                        size={18}
                        color={subDrawerOpen ? drawerColors.activeColor : drawerColors.inactiveColor}
                    />
                </RNView>
                {hovered && !subDrawerOpen && (
